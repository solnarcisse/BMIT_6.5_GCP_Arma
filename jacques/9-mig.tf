...
resource "google_compute_region_instance_group_manager" "region" {
  depends_on         = [google_compute_router_nat.region]
  name               = "${var.app_name}-${var.region}-mig"
  base_instance_name = "${var.app_name}-private-${var.region}"
  region             = var.region

  # Compute zones to be used for VM creation
  distribution_policy_zones = data.google_compute_zones.region.names

  # Instance Template
  version {
    instance_template = google_compute_region_instance_template.region.id
  }
